...
{
	check(ASC);
	UE_LOG(LogTemp, Display, TEXT("Granting Ability Set[%ls]"), *GetNameSafe(this));
	if (!ASC->IsOwnerActorAuthoritative())
	{
		// Must be authoritative to give or take ability sets.
		return;
	...
	// Grant the attribute sets.
	for (int32 SetIndex = 0; SetIndex < AttributeStats.Num(); ++SetIndex)
	{
		const FAbilitySet_Attributes& SetToGrant = AttributeStats[SetIndex];
		ASC->SetNumericAttributeBase(SetToGrant.AttributeSet, SetToGrant.Value);
	}
	...
	for (int32 AbilityIndex = 0; AbilityIndex < GrantedGameplayAbilities.Num(); ++AbilityIndex)
	{
		...
		FGameplayAbilitySpec AbilitySpec(AbilityCDO, AbilityToGrant.AbilityLevel, GetTypeHash(AbilityToGrant.InputTag));
